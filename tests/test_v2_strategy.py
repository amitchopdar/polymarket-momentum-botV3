...
def test_v2_momentum_trigger_and_tp_sl_calculation(memory_db):
    ...
    assert pos["Take_Profit_Price"] == round(0.64 + 0.05, 4)  # $0.69 (+5 cents)
    assert pos["Stop_Loss_Price"] == round(0.65 - 0.10, 4)    # Peak HWM $0.65 -> SL $0.55
...
def test_v2_high_odds_tp_target(memory_db):
    ...
    # Tick now at $0.80 (+0.16 shift >= +0.15, and Entry $0.80 >= $0.75 cutoff) -> Places PENDING_FILL at Limit $0.78
    pos = strat.process_tick(candle_start, slug, "UP", token_id, 0.79, 0.80)
    assert pos is not None

    # Seller hits $0.78 bid -> Fills position to OPEN
    strat.process_tick(candle_start, slug, "UP", token_id, 0.78, 0.79)
    pos = strat.active_position

    assert pos["Entry_Odds"] == 0.80
    assert pos["Take_Profit_Price"] == 0.995  # Fixed $0.995 target for entry >= $0.75
    assert pos["Stop_Loss_Price"] == round(0.79 - 0.10, 4)  # Initial SL from HWM = $0.69
...
def test_v2_tp_and_sl_exits(memory_db):
    ...
    # 1. Place UP position at Ask $0.66 -> Limit $0.64 (PENDING_FILL)
    pos = strat.process_tick(candle_start, slug, "UP", token_id, 0.65, 0.66)
    assert pos is not None

    # 2. Fill position at bid $0.64 -> OPEN (TP = $0.69, SL = $0.55)
    strat.process_tick(candle_start, slug, "UP", token_id, 0.64, 0.65)
    assert strat.active_position["Position_Status"] == "OPEN"

    # 3. Feed tick below SL ($0.54 <= $0.55) -> Instantly closed with STOP_LOSS_HIT
