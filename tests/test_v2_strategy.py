...
def test_v2_minimum_odds_floor_filter(memory_db):
    ...
    strat.tick_buffers["TOK_LOW"] = [(now_sec - 10.0, 0.33, 0.34)]
    ...
    strat.tick_buffers["TOK_HIGH"] = [(now_sec - 10.0, 0.49, 0.50)]
...
def test_v2_momentum_trigger_and_tp_sl_calculation(memory_db):
    ...
    # 2. Feed tick now at $0.66 (+0.21 shift >= +0.20 AND $0.66 >= $0.65) -> Places PENDING_FILL at Limit $0.67 (Ask $0.66 + $0.01)
    ...
    assert pos["Take_Profit_Price"] == round(0.66 + getattr(config, "v2_take_profit_cents", 0.15), 4)
    assert pos["Stop_Loss_Price"] == round(0.66 - getattr(config, "v2_trailing_sl_distance_cents", 0.07), 4)
...
def test_v2_high_odds_tp_target(memory_db):
    ...
    strat.tick_buffers[token_id] = [(now_sec - 10.0, 0.60, 0.61)]

    # Tick now at $0.86 (+0.25 shift >= +0.20) -> Places PENDING_FILL at Limit $0.87 (>= $0.85 cutoff)
    pos = strat.process_tick(candle_start, slug, "UP", token_id, 0.85, 0.86)
    assert pos is not None

    # Market tick fills at Ask $0.86 -> Fills position to OPEN
    strat.process_tick(candle_start, slug, "UP", token_id, 0.85, 0.86)
    pos = strat.active_position

    assert pos["Entry_Odds"] == 0.86
    assert pos["Take_Profit_Price"] == 0.99  # Fixed $0.99 max exchange limit target for entry >= $0.85 cutoff
    assert pos["Stop_Loss_Price"] == round(0.86 - getattr(config, "v2_trailing_sl_distance_cents", 0.07), 4)
...
def test_v2_candle_expiry_rollover(memory_db):
    ...
    now_sec = time.time()
    strat.tick_buffers[token_1] = [(now_sec - 10.0, 0.44, 0.45)]
    ...
    strat.tick_buffers[token_2] = [(now_sec - 10.0, 0.44, 0.45)]
...
def test_v2_mid_candle_dip_below_sl_and_expiry_validation(memory_db):
    ...
    now_sec = time.time()
    strat.tick_buffers[token_id] = [(now_sec - 10.0, 0.59, 0.60)]

    # Enter position at Ask $0.80 -> Limit $0.81 (PENDING_FILL)
    pos = strat.process_tick(candle_1, slug_1, "UP", token_id, 0.79, 0.80)
    assert pos is not None

    # Fill position at bid $0.78 -> OPEN (SL = $0.69)
    strat.process_tick(candle_1, slug_1, "UP", token_id, 0.79, 0.80)
    assert strat.active_position["Position_Status"] == "OPEN"

    # Mid-candle tick dips to $0.70 (breaching SL $0.73)
    strat.process_tick(candle_1, slug_1, "UP", token_id, 0.70, 0.71)
...
def test_v2_trailing_stop_loss_hwm(memory_db):
    ...
    pos = strat.execute_entry_v2(candle, slug, "UP", token_id, 0.44, 0.66)
    pos["Take_Profit_Price"] = 0.995
    strat.active_position = pos

    sl_dist = getattr(config, "v2_trailing_sl_distance_cents", 0.07)
    assert pos["Stop_Loss_Price"] == round(0.66 - sl_dist, 4)
    ...
    assert strat.active_position["Stop_Loss_Price"] == round(0.69 - sl_dist, 4)
...
def test_v2_high_odds_trailing_sl(memory_db):
    ...
    # Entry at Ask $0.80 -> Limit $0.81 (PENDING_FILL)
